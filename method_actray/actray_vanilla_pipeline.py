...
@dataclass
class ActRayVanillaPipelineConfig(VanillaPipelineConfig):
...
class ActRayVanillaPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        if type(self.datamanager.dataparser).__name__ == 'Blender':
            ...
            self.total_ray_num = (tmp == 1e30).sum()
    ...
    def _projection(self, surface_points: torch.Tensor, step):
        ...
        norm_view_space_coords_x = view_space_coords[..., 0] / (-view_space_coords[..., 2]) # 归一化相机坐标系
    ...
    def _update_active_pixel_samplers(self, outputs_rgb: torch.Tensor, outputs_depth: torch.Tensor, ray_bundle: RayBundle, batch: torch.Tensor, pic_space_coords_ray_bundle: torch.Tensor, step: int):
        ...
        with torch.no_grad():
            ...
            loss_new = (loss_ray.unsqueeze(1).repeat(1, image_num).view(-1))[seen_mask] # ray_num * 1 -> ray_num * image_num -> (ray_num * image_num)
            ...
            propogation_success_mask = (similarity_new > 0)
            pic_space_idx = pic_space_idx[propogation_success_mask]
            similarity_new = similarity_new[propogation_success_mask]
            loss_new = loss_new[propogation_success_mask]
            ...
            if need_to_stop == True:
                ...
                self.datamanager.train_pixel_sampler.loss_gzero_count += unpropagate_mask.sum()
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if self.config.datamanager.camera_optimizer is not None: # 对instant而言不训练相机参数
